...
export class TimeCellTemplate implements CellTemplate<TimeCell> {
  ...
  getCompatibleCell(uncertainCell: Uncertain<TimeCell>): Compatible<TimeCell> {
    const time = uncertainCell.time
      ? getCellProperty(uncertainCell, 'time', 'object')
      : new Date(NaN)
  ...
  update(
    cell: Compatible<TimeCell>,
    cellToMerge: UncertainCompatible<TimeCell>
  ): Compatible<TimeCell> {
      ...
      <input
        ...
        onChange={(e) => {
          ...
          if (!Number.isNaN(timestamp))
            onCellChanged(this.getCompatibleCell({ ...cell, time: new Date(timestamp) }), false)
        }}
        onBlur={(e) => {
          const timestamp = getTimestamp(e.currentTarget.value)
          if (!Number.isNaN(timestamp)) {
            onCellChanged(
              this.getCompatibleCell({ ...cell, time: new Date(timestamp) }),
