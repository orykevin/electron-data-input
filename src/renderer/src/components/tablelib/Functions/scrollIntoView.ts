...
export function scrollIntoView(state: State, top: number, left: number): void {
  ...
  scrollableElement.scrollTop !== undefined
    ? (scrollableElement.scrollTop = top)
    : scrollableElement.scrollTo({ top })
  scrollableElement.scrollLeft !== undefined
    ? (scrollableElement.scrollLeft = left)
    : scrollableElement.scrollTo({ left })
}
...
export function getCalculatedScrollTopValueToBottom(
  location: Location,
  visibleScrollAreaHeight: number,
  scrollTop: number,
...
export function getCalculatedScrollTopValueToTop(
  location: Location,
  scrollTop: number,
  topStickyOffset: number
): number {
...
export function isBottomCellAllVisible(
  state: State,
  location: Location,
  visibleScrollAreaHeight: number
): boolean {
...
export function getCalculatedScrollLeftValueToRight(
  location: Location,
  visibleScrollAreaWidth: number,
  scrollLeft: number,
  leftStickyOffset: number
): number {
  return scrollLeft + location.column.right - visibleScrollAreaWidth - leftStickyOffset
}

export function getCalculatedScrollLeftValueToLeft(
  location: Location,
  scrollLeft: number,
  leftStickyOffset: number
): number {
...
export function isRightCellAllVisible(
  state: State,
  location: Location,
  visibleScrollAreaWidth: number
): boolean {
