...
export function getScrollTop(state: State, location: Location, dontChange: boolean): number {
  ...
  const additionalPixelOnScrollingOnBody = isLocationOnScrollableBody(state, location) ? 1 : 0
  if (
...
export function getScrollLeft(state: State, location: Location, dontChange: boolean): number {
...
function isFocusLocationOnRightSticky(state: State, location: Location) {
  ...
  const column = location.column
  return stickyRightRange.columns.length > 0 && column.idx >= stickyRightRange.first.column.idx
}

function isFocusLocationOnBottomSticky(state: State, location: Location) {
  const { stickyBottomRange } = state.cellMatrix.ranges
  const row = location.row
  return stickyBottomRange.rows.length > 0 && row.idx >= stickyBottomRange.first.row.idx
}
