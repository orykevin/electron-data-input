...
export type FocusLocationFn = (
  state: State,
  location: Location,
  applyResetSelection: boolean,
  keyCode?: keyCodes
) => State
export type FocusCellFn = (
  colIdx: number,
  rowIdx: number,
  state: State,
  keyCode?: keyCodes
  ...
  return (state: State): State => {
    if (state.focusedLocation) {
      const nextFocusableLocation = getNextFocusableLocation(
        ...
        state.cellMatrix.columns.length - 1
      )
...
export function getNextFocusableLocation(
  state: State,
  rowIdx: number,
  colIdx: number
): Location | undefined {
  ...
  const isFocusable =
    (!cellTemplate.isFocusable || cellTemplate.isFocusable(cell)) && wasChangePrevented
  return isFocusable ? location : undefined
}

export function getFocusLocationToLeft(
  state: State,
  location: Location | undefined
): Location | undefined {
  if (location) {
    for (
      let colIdx = location.column.idx - 1;
      colIdx >= state.cellMatrix.first.column.idx;
      --colIdx
    ) {
...
export function withMoveFocusRight(fc: FocusCellFn) {
  return (state: State, keyCode?: keyCodes): State => {
    ...
    return focusLocation
      ? fc(focusLocation.column.idx, focusLocation.row.idx, state, keyCode)
      : state
  }
}

export function getFocusLocationToRight(
  state: State,
  location: Location | undefined
): Location | undefined {
  if (location) {
    for (
      let colIdx = location.column.idx + 1;
      colIdx <= state.cellMatrix.last.column.idx;
      ++colIdx
    ) {
      const nextFocusableLocation = getNextFocusableLocation(state, location.row.idx, colIdx)
      if (nextFocusableLocation) {
...
export function getFocusLocationToUp(
  state: State,
  location: Location | undefined
): Location | undefined {
  if (location) {
...
export function getFocusLocationToDown(
  state: State,
  location: Location | undefined
): Location | undefined {
  if (location) {
...
function pageDownRowCalc(state: State, location: Location): number {
  ...
  const isOnLastRowOnTopSticky =
    hasTopSticky && location.row.idx === state.cellMatrix.ranges?.stickyTopRange.last.row.idx
  ...
  const isOnLastRowOfScrollableRange =
    hasScrollableRange && location.row.idx === state.cellMatrix.scrollableRange.last.row.idx
  const hasBottomSticky = state.cellMatrix.ranges.stickyBottomRange.rows.length > 0
  const isOnBottomSticky =
    hasBottomSticky && location.row.idx >= state.cellMatrix.ranges.stickyBottomRange.first.row.idx
  const visibleScrollAreaHeight = getVisibleHeight(
    state,
    state.cellMatrix.ranges.stickyTopRange.height + state.cellMatrix.ranges.stickyBottomRange.height
  )
  ...
  if (isOnTopSticky) {
    ...
    rowIdx =
      location.row.idx + rowsOnScreen.length < state.cellMatrix.scrollableRange.rows.length
...
function pageUpRowCalc(state: State, location: Location): number {
  ...
  const isOnFirstElementOnScrollableRange =
    hasScrollableRange && location.row.idx === state.cellMatrix.scrollableRange.first.row.idx
  const hasBottomSticky = state.cellMatrix.ranges.stickyBottomRange.rows.length > 0
  const isOnBottomSticky =
    hasBottomSticky && location.row.idx >= state.cellMatrix.ranges.stickyBottomRange.first.row.idx
  ...
  if (isOnBottomSticky) {
    ...
    rowIdx =
      location.row.idx - rowsOnScreen.length < state.cellMatrix.scrollableRange.first.row.idx
