...
function handleKeyDownInternal(state: State, event: KeyboardEvent): State {
  ...
  if (newState !== state) {
    if (!isSingleCellSelected && event.keyCode === keyCodes.ENTER) {
      const direction = event.shiftKey ? 'up' : state.props?.moveRightOnEnter ? 'right' : 'down'
      state.hiddenFocusElement?.focus()
      return moveFocusInsideSelectedRange(state, direction, asr, location) as State
    }
    return newState
  }
...
function moveFocusInsideSelectedRange(
  ...
  const onTabKey =
    ...
    (newPosInRange === 0 &&
      currentPosInRange === asr.rows.length * asr.columns.length - 1 &&
      ((rowCount === 2 && colCount >= 1) || (rowCount >= 1 && colCount === 2)) &&
      delta === 1) ||
    (newPosInRange < 0 && currentPosInRange === 0) ||
...
function resizeSelectionPageDown(state: State, asr: Range, location: Location): State {
...
function resizeSelectionPageUp(state: State, asr: Range, location: Location): State {
  const visibleScrollAreaHeight = getVisibleHeight(state)
  ...
  if (scrollDirection) {
    ...
    switch (scrollDirection) {
      case 'horizontal':
        ...
        scrollToColIdx = location.column.idx !== firstColIdx ? firstColIdx : lastColIdx
        break
      case 'vertical':
        scrollToRowIdx = location.row.idx !== firstRowIdx ? firstRowIdx : lastRowIdx
