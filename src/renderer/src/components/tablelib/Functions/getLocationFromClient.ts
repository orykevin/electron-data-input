...
function getRowOnNonSticky(state: State, viewportY: number): { cellY: number; row: GridRow } {
  if (state.cellMatrix.scrollableRange.rows.length < 1) {
    const sticky =
      viewportY >= state.cellMatrix.height ? state.cellMatrix.last : state.cellMatrix.first
    return {
...
function getColumnOnNonSticky(
  ...
  if (state.cellMatrix.scrollableRange.columns.length < 1) {
    const sticky =
      viewportX >= state.cellMatrix.width ? state.cellMatrix.last : state.cellMatrix.first
    return {
...
export function getStickyTopRow(
  ...
  const topStickyOffset = getStickyOffset(scrollTop, top)
  if (
    cellMatrix.ranges.stickyTopRange.rows.find((row) => row.bottom > viewportY - topStickyOffset) &&
...
export function getScrollableContentRow(
  ...
  const row =
    cellMatrix.scrollableRange.rows.find((row) => row.bottom >= scrollableContentY) ||
...
export function getScrollableContentColumn(
  ...
  const column =
    cellMatrix.scrollableRange.columns.find((column) => column.right >= scrollableContentX) ||
