...
export function selectRange(state: State, range: Range, incremental: boolean): State {
  return {
    ...
    activeSelectedRangeIdx:
      incremental && state.selectionMode === 'range' ? state.selectedRanges.length : 0
  }
}
...
export function selectOneColumn(state: State, col: GridColumn, incremental: boolean): State {
  return {
    ...
    selectedIds: (incremental && state.selectionMode === 'column' ? state.selectedIds : []).concat(
      col.columnId
    )
  }
}
...
export function unSelectOneColumn(state: State, col: GridColumn): State {
  const updatedIndexes = state.selectedIndexes.filter((idx: number) => idx !== col.idx)
...
export function selectOneRow(state: State, row: GridRow, incremental: boolean): State {
  return {
    ...
    selectedIds: (incremental && state.selectionMode === 'row' ? state.selectedIds : []).concat(
      row.rowId
    )
  }
}
...
export function unSelectOneRow(state: State, row: GridRow): State {
  const updatedIndexes = state.selectedIndexes.filter((idx: number) => idx !== row.idx)
