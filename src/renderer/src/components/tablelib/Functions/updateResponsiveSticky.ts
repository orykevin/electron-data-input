...
export function updateResponsiveSticky(props: ReactGridProps, state: State): State {
  ...
  if (
    ...
    const { width: widthOfScrollableElement, height: heightOfScrollableElement } = getSizeOfElement(
    ...
    if (props.stickyLeftColumns || props.stickyRightColumns) {
      ...
      rightStickyColumns = shouldDisableStickyHorizontally ? 0 : rightStickyColumns
    }
    if (props.stickyTopRows || props.stickyBottomRows) {
      const predictedTopRangeHeight = props.rows.slice(0, topStickyRows).reduce((acc, column) => {
        return acc + (column.height || CellMatrix.DEFAULT_ROW_HEIGHT)
      }, 0)
      let predictedBottomRangeHeight = 0
      if (bottomStickyRows > 0) {
        predictedBottomRangeHeight = props.rows.slice(-bottomStickyRows).reduce((prev, column) => {
