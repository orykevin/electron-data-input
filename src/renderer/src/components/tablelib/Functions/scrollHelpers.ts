...
export function getScrollOfScrollableElement(element: ScrollableElement): {
  scrollLeft: number
  scrollTop: number
} {
  const scrollLeft =
    element !== undefined
      ? ((element as HTMLElement).scrollLeft ?? getTopScrollableElement().scrollX) -
        ((element as HTMLElement).clientLeft || 0)
      : 0
  const scrollTop =
    element !== undefined
      ? ((element as HTMLElement).scrollTop ?? getTopScrollableElement().scrollY) -
        ((element as HTMLElement).clientTop || 0)
      : 0
