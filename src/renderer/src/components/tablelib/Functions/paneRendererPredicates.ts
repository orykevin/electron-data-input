...
export function shouldRenderMiddleRange(state: State): boolean {
  return !!(
    state.cellMatrix.scrollableRange.height > 0 &&
    state.cellMatrix.scrollableRange.first.column &&
    state.cellMatrix.scrollableRange.first.row &&
    state.cellMatrix.scrollableRange.last.row &&
    state.visibleRange &&
...
export function shouldRenderBottomSticky(state: State): boolean {
  return !!(
    state.cellMatrix.ranges.stickyBottomRange.height > 0 && state.cellMatrix.rows.length > 0
  )
}
