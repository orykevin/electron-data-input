...
export class ResizeColumnBehavior extends Behavior {
  ...
  getLinePositionOffset(state: State): number {
    ...
    let offset = 0
    if (state.cellMatrix.scrollableRange.columns.some((c) => c.idx === this.resizedColumn.idx)) {
      offset = state.cellMatrix.ranges.stickyLeftRange.width
    } else if (
      state.cellMatrix.ranges.stickyRightRange.columns.some((c) => c.idx === this.resizedColumn.idx)
    ) {
