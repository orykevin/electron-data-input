import { Direction, PointerLocation, getScrollOfScrollableElement } from '../../core'
...
export class ColumnReorderBehavior extends Behavior {
  ...
  handlePointerEnter(event: PointerEvent, location: PointerLocation, state: State): State {
    ...
    const linePosition = Math.min(
      dropLocation.viewportX - dropLocation.cellX + (drawRight ? dropLocation.column.width : 0),
  ...
  getLastPossibleDropLocation(
    ...
    if (
      ...
      state.props.canReorderColumns(currentLocation.column.columnId, columnIds, position)
    ) {
