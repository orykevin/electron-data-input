...
export class FillHandleBehavior extends Behavior {
  ...
  handlePointerUp(event: PointerEvent, location: Location, state: State): State {
    ...
    const getCompatibleCell = (location: Location) => getCompatibleCellAndTemplate(state, location)
    ...
    const fillCellsWithPredictedValues = (
      ...
      return cellsToFill.map((cell, i) => {
        ...
        return {
          ...
          ...(cell.type === 'checkbox' && {
            checked: 'checked' in selectedCell ? selectedCell.checked : !!selectedCell.value
          }),
          text: areParametersNaNs || isKeyPressed ? selectedCell.text : x.toString(),
    ...
    const fillVertically = (
      ...
      activeSelectedRange.columns.forEach((column) => {
        ...
        selectedCells = direction === 'up' ? selectedCells.reverse() : selectedCells
        if (this.fillRange) {
          let cellsToFill = this.fillRange.rows.map(
            (row) => getCompatibleCell(newLocation(row, column)).cell
          )
          cellsToFill = fillCellsWithPredictedValues(selectedCells, cellsToFill)
          cellsToFill = direction === 'up' ? cellsToFill.reverse() : cellsToFill
    ...
    const fillHorizontally = (
      ...
      activeSelectedRange.rows.forEach((row) => {
        ...
        selectedCells = direction === 'left' ? selectedCells.reverse() : selectedCells
        if (this.fillRange) {
          let cellsToFill = this.fillRange.columns.map(
            (column) => getCompatibleCell(newLocation(row, column)).cell
          )
          cellsToFill = fillCellsWithPredictedValues(selectedCells, cellsToFill)
          cellsToFill = direction === 'left' ? cellsToFill.reverse() : cellsToFill
  ...
  findRegressionFunction(valuesX: number[], valuesY: number[]): { a: number; b: number } {
  ...
  private fillRow(state: State, row: GridRow, values: Compatible<Cell>[]): State {
    this.fillRange?.columns.forEach((col, i) => {
      state = tryAppendChangeHavingGroupId(state, newLocation(row, col), values[i]) as State
    })
    return state
  }

  private fillColumn(state: State, column: GridColumn, values: Compatible<Cell>[]): State {
    this.fillRange?.rows.forEach((row, i) => {
      state = tryAppendChangeHavingGroupId(state, newLocation(row, column), values[i]) as State
    })
    return state
  }
}
