...
export abstract class AbstractPointerEventsController {
  ...
  protected handlePointerDownInternal(
    event: PointerEvent,
    ...
    if (
      ...
      isFirstRowOrColumn ||
      areLocationsEqual(currentLocation, previousLocation)
    ) {
  ...
  protected shouldHandleDoubleClick(
    currentLocation: PointerLocation,
    currentTimestamp: number,
    secondLastTimestamp: number
  ): boolean {
    return (
      currentTimestamp - secondLastTimestamp < 500 &&
      areLocationsEqual(currentLocation, this.eventLocations[0]) &&
      areLocationsEqual(currentLocation, this.eventLocations[1])
    )
  }

  protected shouldHandleCellSelectionOnMobile(
    event: PointerEvent,
    currentLocation: PointerLocation,
    currentTimestamp: number
  ): boolean {
    ...
    return (
      ...
      currentLocation.row.idx > 0 &&
      currentLocation.column.idx > 0
    )
  }
}
