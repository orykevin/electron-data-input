...
export class EventHandlers {
  ...
  pointerDownHandler = (event: PointerEvent): void =>
    this.updateState((state) => this.pointerEventsController.handlePointerDown(event, state))
  keyDownHandler = (event: KeyboardEvent): void =>
    this.updateState((state) => state.currentBehavior.handleKeyDown(event, state))
  keyUpHandler = (event: KeyboardEvent): void =>
    this.updateState((state) => state.currentBehavior.handleKeyUp(event, state))
  compositionEndHandler = (event: CompositionEvent): void =>
    this.updateState((state) => state.currentBehavior.handleCompositionEnd(event, state))
  ...
  blurHandler = (event: FocusEvent): void =>
    this.updateState((state) => {
      if ((event.target as HTMLInputElement)?.id?.startsWith('react-select-')) {
        // give back focus on react-select dropdown blur
  ...
  reactgridRefHandler = (reactGridElement: HTMLDivElement): void =>
    this.assignElementsRefs(reactGridElement, recalcVisibleRange)
  ...
  pasteCaptureHandler = (event: ClipboardEvent): void => {
    ...
    if (
      htmlData &&
      parsedData.body.firstElementChild?.getAttribute('data-reactgrid') === 'reactgrid-content'
    ) {
      event.bubbles = false
    }
  }
  ...
  protected assignElementsRefs = (
    reactGridElement: HTMLDivElement,
    visibleRangeCalculator: StateModifier
  ): void => {
    if (reactGridElement) {
      this.updateState((state) => {
  ...
  protected updateOnScrollChange = (visibleRangeCalculator: StateModifier): void => {
    this.updateState((state) => {
      ...
      const shouldBeVisibleRangeRecalc =
        width > 0 &&
        height > 0 &&
        (scrollTop >= state.bottomScrollBoudary - PAGE_UPDATE_OFFSET ||
          scrollTop <= state.topScrollBoudary + PAGE_UPDATE_OFFSET ||
          scrollLeft >= state.rightScrollBoudary - PAGE_UPDATE_OFFSET ||
          scrollLeft <= state.leftScrollBoudary + PAGE_UPDATE_OFFSET)
