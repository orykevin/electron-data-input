...
export class CellMatrixBuilder implements ICellMatrixBuilder {
  ...
  fillRowsAndCols(
    ...
    const { leftStickyColumns, topStickyRows, rightStickyColumns, bottomStickyRows } = edges
    ...
    this.cellMatrix.rows = this.cellMatrix.props.rows.reduce<GridRow[]>((rows, row, idx) => {
    ...
    this.cellMatrix.columns = this.cellMatrix.props.columns.reduce<GridColumn[]>(
      (cols, column, idx) => {
        const left = this.getLeft(idx, leftStickyColumns, rightStickyColumns, cols)
  ...
  setRangesToRenderLookup(): CellMatrixBuilder {
    ...
    const keys = rangesToExclude.map((range) =>
      translateLocationIdxToLookupKey(range.first.column.idx, range.first.row.idx)
    )
  ...
  private getTop(
    ...
    return idx === 0 ||
      idx === topRowsSticky ||
      idx === this.getStickyBottomFirstIdx(topRowsSticky || 0, bottomRowsSticky || 0)
      ? 0
  ...
  private getLeft(
    ...
    return idx === 0 ||
      ...
      idx === this.getStickyRightFirstIdx(leftColumnsSticky || 0, rightColumnsSticky || 0)
      ? 0
  ...
  private getStickyBottomFirstIdx(topRowsSticky: number, bottomRowsSticky: number): number {
    const stickyBottomRows = bottomRowsSticky || 0
    const stickyTopRows = topRowsSticky || 0
    const rows = this.cellMatrix.props.rows.length
    return rows - (stickyBottomRows + stickyTopRows > rows ? 0 : stickyBottomRows)
  }

  private getStickyRightFirstIdx(stickyColumnsLeft: number, stickyColumnsRight: number): number {
    ...
    return columns - (stickyRightColumns + stickyLeftColumns > columns ? 0 : stickyRightColumns)
  }
