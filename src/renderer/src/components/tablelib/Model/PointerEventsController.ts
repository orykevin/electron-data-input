...
} from './AbstractPointerEventsController'
...
export class PointerEventsController extends AbstractPointerEventsController {
  ...
  public handlePointerDown = (event: PointerEvent, state: State): State => {
    ...
    const onClickableAreaOnPro = isOnClickableArea(event, state as State)
  ...
  private handleContextMenu = (event: PointerEvent | MouseEvent): void => {
    ...
    this.updateState((state) => {
      if (this.isContainElement(event, state)) {
        const currentLocation = getLocationFromClient(state as State, event.clientX, event.clientY)
        state = (state.currentBehavior as Behavior).handlePointerUp(event, currentLocation, state)
        state = (state.currentBehavior as Behavior).handleContextMenu(event, state as State)
  ...
  private handlePointerMove = (event: PointerEvent): void => {
    this.updateState((state) => {
      const autoScrollDirection = (state.currentBehavior as Behavior).autoScrollDirection
      ...
      if (
        ...
        const { left, top } = scrollCalculator(state as State, currentLocation)
