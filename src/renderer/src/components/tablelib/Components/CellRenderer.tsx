...
import { ResizeColumnHandle, ResizeRowHandle } from './ResizeHandle'
...
export const CellRenderer: React.FC<CellRendererProps> = ({
  ...
  const isFocused =
    state.focusedLocation !== undefined && areLocationsEqual(state.focusedLocation, location)
  ...
  const style = {
    ...
    ...((isFocused || cell.type === 'header' || isFirstRowOrColumnWithSelection) && {
      touchAction: 'none'
    }) // prevent scrolling
  ...
  return (
    ...
    >
      ...
      {location.row.idx === 0 ||
        (cell.type === 'header' &&
          state.props?.enableColumnResizeOnAllHeaders &&
          location.column.resizable && <ResizeColumnHandle />)}
