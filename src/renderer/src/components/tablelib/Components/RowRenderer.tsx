...
const MappedColumns: React.FC<RowRendererProps> = ({
  ...
  return (
    <>
      {columns.map((column) => {
        const range =
          state.cellMatrix.rangesToRender[translateLocationIdxToLookupKey(column.idx, row.idx)]
            ?.range
        if (!range) {
          return null
        }
          ...
          <CellRenderer
            ...
            borders={{
              ...
              right:
                ...
                !(state.cellMatrix.scrollableRange.last.column.idx === location.column.idx)
            }}
            state={state}
            location={location}
            range={range}
...
export const RowRenderer: React.NamedExoticComponent<RowRendererProps> = React.memo(
  MappedColumns,
