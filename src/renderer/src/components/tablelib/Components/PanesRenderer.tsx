...
export const PanesRenderer: React.FC<PanesProps> = ({ cellRenderer }) => {
  ...
  const marginTopOnScrollable =
    cellMatrix.scrollableRange.rows.length !== 0 ? cellMatrix.ranges.stickyTopRange.height : 0
  const marginLeftOnScrollable =
    cellMatrix.scrollableRange.columns.length !== 0 ? cellMatrix.ranges.stickyLeftRange.width : 0
  const marginBottomOnScrollable =
    cellMatrix.scrollableRange.rows.length !== 0 ? cellMatrix.ranges.stickyBottomRange.height : 0
  const marginRightOnScrollable =
    cellMatrix.scrollableRange.columns.length !== 0 ? cellMatrix.ranges.stickyRightRange.width : 0
  ...
  return (
    <>
      ...
      <PaneShadow
        ...
        zIndex={1}
        style={{
          width: state.props?.enableFullWidthHeader ? `calc(100%)` : cellMatrix.width,
